...
return_values_init bsec_iot_init(float sample_rate, float temperature_offset, bme68x_write_fptr_t bus_write,
                                 bme68x_read_fptr_t bus_read, sleep_fct sleep, state_load_fct state_load,
    ...
    printf("Initializing BME68x API chip_id %d.. \n", bme68x_g.chip_id);
    printf("Initializing BME68x API Sample Rate %d.. \n", sample_rate);
    printf("Initializing BME68x API intf_ptr %d.. \n", (int) bme68x_g.intf_ptr);
    printf("Initializing BME68x API intf %d.. \n", (int) bme68x_g.intf);
    printf("Initializing BME68x API intf %d.. \n", (int) bme68x_g.mem_page);

    /* Initialize bme68x API */
    ret.bme68x_status = bme68x_init(&bme68x_g);
    if (ret.bme68x_status != BME68X_OK) {
        printf("Initializing BME68x bme68x API Failed! %d .. \n", ret.bme68x_status);
        return ret;
    }

    printf("Initializing BME68x Library .. \n");
    /* Initialize BSEC library */
    ret.bsec_status = bsec_init();
    if (ret.bsec_status != BSEC_OK) {
        printf("Initializing BME68x Library Failed  %d.. \n", ret.bsec_status);
        return ret;
    }

    printf("Initializing BME68x  Library Config Loading .. \n");
    ...
    if (bsec_config_len != 0) {
        ...
        if (ret.bsec_status != BSEC_OK) {
            printf("Initializing BME68x Library Config Loading Failed %d .. \n", ret.bsec_status);
            return ret;
        }
    }

    printf("Initializing BME68x Load previous library state, if available .. \n");
    ...
    if (bsec_state_len != 0) {
        ...
        if (ret.bsec_status != BSEC_OK) {
            printf("Initializing BME68x Load previous library state , Failed.. %d \n", ret.bsec_status);
            return ret;
        }
    }

    printf("Initializing BME68x Call to the function which sets the library with subscription information .. \n");
    ...
    if (ret.bsec_status != BSEC_OK) {
        printf("Initializing BME68x bme68x_bsec_update_subscription failed %d .. \n", ret.bsec_status);
        return ret;
    }
...
bme68x_bsec_process_data(bsec_input_t *bsec_inputs, uint8_t num_bsec_inputs, output_ready_fct output_ready) {
    ...
    if (num_bsec_inputs > 0) {
        ...
        output_ready(timestamp, iaq, static_iaq, temp, raw_temp, humidity, raw_humidity, raw_pressure,
                     raw_gas, gas_percentage, co2_equivalent, breath_voc_equivalent, bsec_status,
                     stabStatus, runInStatus);
 ...
 */
uint8_t
processData(int64_t currTimeNs, struct bme68x_data data, int32_t bsec_process_data, output_ready_fct output_ready) {
    ...
    while (1) {
        ...
        if (time_stamp >= sensor_settings.next_call) {
            ...
            if (n_samples >= save_intvl) {
                bsec_status = bsec_get_state(0, bsec_state, sizeof(bsec_state), work_buffer, sizeof(work_buffer),
                                             &bsec_state_len);
                if (bsec_status == BSEC_OK) {
